// src/pages/Client/FlowerDecorations.js
...
const FlowerDecorations = () => {
    const { decorations, loading, error } = useDecorations("Flower Decoration");
    ...
    if (loading) return <p style={{textAlign: 'center', padding: '100px'}}>Loading Flower Decorations...</p>;
    if (error) return <p style={{textAlign: 'center', padding: '100px', color: 'red'}}>{error}</p>;

    return (
        <div style={{ padding: '75px' }}>
           <center><h2 style={{textAlign: 'center', color: 'red'}}>Flower Decoration Portfolio</h2></center> 
          <center> <h7>To view the image in full size, click on the image.</h7></center> 
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
                {decorations.length > 0 ? (
                    ...
                    <p>No flower decorations added yet.</p>
                )}
            </div>
